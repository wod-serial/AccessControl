 ...
 * Version 3.0 to 4.1 by Aleš Kapica (git branch form-support)
 * Version 5.x This version was skipped because there were unsupported forks based a branch version 2.6, versioned as 5.x
 *
 * @package MediaWiki
 * @subpackage Extensions
 * @author Aleš Kapica
 * @copyright 2008-2023 Aleš Kapica
	...
	*/
	...
	private static function testRightsArray( $rights ) {
		...
		if ( array_key_exists( $user->getName(), $rights[EDIT] ) || array_key_exists( $user->getName(), $rights[VIEW] ) ) {
			if ( array_key_exists( $user->getName(), $rights[EDIT] ) && $rights[EDIT][$user->getName()] ) {
				return 1;
			} else {
